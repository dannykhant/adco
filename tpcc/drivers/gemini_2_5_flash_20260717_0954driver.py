...
# Optimized TXN_QUERIES dict
# Applying COMBINING_QUERIES, PREDICATE_PUSHDOWN, JOIN_ORDER_HINTS, CONCURRENCY
TXN_QUERIES = {
    "DELIVERY": {
        "batchNewOrders": """
            SELECT n.NO_D_ID, n.NO_O_ID, o.O_C_ID,
                   COALESCE((SELECT SUM(OL_AMOUNT) FROM ORDER_LINE
                             WHERE OL_O_ID = n.NO_O_ID AND OL_D_ID = n.NO_D_ID
                               AND OL_W_ID = o.O_W_ID), 0.0) AS ol_total
    ...
    "NEW_ORDER": {
        ...
        # Optimized with STRAIGHT_JOIN and corrected join condition for CUSTOMER
        "getMiscInfo": """
    ...
    "ORDER_STATUS": {
        # v2's combined query for customer, last order, and order lines is already highly optimized.
        # Uses LEFT JOIN with a derived table for the last order and then another LEFT JOIN for order lines.
        ...
        # v2's combined query for last order and order lines.
    ...
    "PAYMENT": {
        # Optimized with STRAIGHT_JOIN for better performance hint
        ...
        # v2's combined query for warehouse and district is efficient
    ...
    "STOCK_LEVEL": {
        # v2's combined query with derived table for predicate pushdown is highly optimized
        "getStockCount": """
...
class Gemini_2_5_Flash_20260717_0954Driver(AbstractDriver):
    DEFAULT_CONFIG = {
    ...
    def __init__(self, ddl):
        ...
        if os.path.exists(mysql_ddl):
            ddl = mysql_ddl
        super(Gemini_2_5_Flash_20260717_0954Driver, self).__init__("candidates", ddl)
        self.host = None
        self.port = None
        self.user = None
        self.password = None
    ...
    def makeDefaultConfig(self):
        return Gemini_2_5_Flash_20260717_0954Driver.DEFAULT_CONFIG

    def loadConfig(self, config):
        for key in Gemini_2_5_Flash_20260717_0954Driver.DEFAULT_CONFIG:
    ...
    # --- Batch Helper Functions (Copied verbatim from v2) ---
    ...
    def _batch_update_stock(self, updates):
        ...
        # IMPORTANT: Batch update stock params are column-major as specified
    ...
    def doDelivery(self, params):
        ...
        # COMBINING_QUERIES: Fetch all pending new orders for all districts at once
        self.cursor.execute(q["batchNewOrders"], [w_id, w_id])
        rows = self.cursor.fetchall()

        result = []
        if rows:
            pairs_to_delete = []  # (d_id, no_o_id) for NEW_ORDER, ORDERS, ORDER_LINE updates
            customer_balance_updates = [] # (c_id, d_id, ol_total) for CUSTOMER updates
            for row in rows:
                d_id, no_o_id, c_id, ol_total = row
                # ol_total could be 0.0 if no order lines (COALESCE handles this).
                # TPC-C spec implies there will always be order lines.
                assert ol_total is not None, "ol_total is NULL: there are no order lines"
                assert ol_total >= 0.0 # TPC-C 2.7.2.1.F sum(OL_AMOUNT) must be >= 0

                pairs_to_delete.append((d_id, no_o_id))
                customer_balance_updates.append((c_id, d_id, ol_total))
                result.append((d_id, no_o_id))

            # SEPARATING_QUERIES & CONCURRENCY: Perform batch updates for DMLs
            self._batch_delete_new_orders(w_id, pairs_to_delete)
            self._batch_update_orders(o_carrier_id, w_id, pairs_to_delete)
            self._batch_update_order_lines(ol_delivery_d, w_id, pairs_to_delete)
            self._batch_update_customers(w_id, customer_balance_updates)
    ...
    def doNewOrder(self, params):
        ...
        # CONCURRENCY: Batch item info fetch
        items_map = self._batch_items(i_ids)

        # Check if all items exist
        for i_id in i_ids:
            if i_id not in items_map:
                logging.warning(f"ITEM {i_id} not found. Aborting NewOrder transaction.")
                self.conn.rollback()
                return
        ...
        customer_info_result = misc_row[3:6] # c_discount, c_last, c_credit, for return value

        ol_cnt = len(i_ids)
        o_carrier_id = constants.NULL_CARRIER_ID

        # Sequential DMLs - increment D_NEXT_O_ID, create ORDER and NEW_ORDER entries
        ...
        # CONCURRENCY: Batch stock info fetch
        ...
        for i in range(len(i_ids)):
            ...
            if stock_key not in stock_map:
                logging.warning(f"No STOCK record for (ol_i_id={ol_i_id}, ol_supply_w_id={ol_supply_w_id}). Aborting NewOrder transaction.")
                self.conn.rollback()
                return
            ...
            # TPC-C 2.5.1.3 Stock Quantity Update Logic
            s_ytd += ol_quantity
        ...
        misc_for_return = [(w_tax, d_tax, d_next_o_id, total_amount)]

        return [customer_info_result, misc_for_return, item_data_for_return]
    ...
    def doOrderStatus(self, params):
        ...
        if c_id is not None:
            # COMBINING_QUERIES: Fetch customer, last order, and all order lines in one query
            self.cursor.execute(q["getCustomerByIdWithLines"], [w_id, d_id, c_id, w_id, d_id, c_id])
            rows = self.cursor.fetchall()
            if rows:
                customer = rows[0][:5]
                if rows[0][5] is not None: # Check if an order was found
                    order = rows[0][5:8]
                    orderLines = tuple((r[8], r[9], r[10], r[11], r[12]) for r in rows if r[8] is not None)
        else:
            # First fetch customers by last name to find the "median" C_ID
            self.cursor.execute(q["getCustomersByLastName"], [w_id, d_id, c_last])
            all_customers = self.cursor.fetchall()
            assert len(all_customers) > 0, f"No customer found for C_LAST='{c_last}'"
        ...
        assert customer is not None, f"Customer could not be determined for W_ID={w_id}, D_ID={d_id}, C_ID={c_id} or C_LAST={c_last}"
    ...
    def doPayment(self, params):
        ...
        if c_id is not None:
            ...
            assert row is not None, f"Customer not found for C_W_ID={w_id}, C_D_ID={d_id}, C_ID={c_id}"
            customer = row[:18]
            warehouse = row[18:24]
            district = row[24:]
        else:
            ...
            index = (namecnt - 1) // 2
            customer = all_customers[index]
            c_id = customer[0] # Get the specific C_ID for later updates

            # COMBINING_QUERIES: Fetch Warehouse and District info
            self.cursor.execute(q["getWarehouseAndDistrict"], [d_id, w_id])
            row = self.cursor.fetchone()
            assert row is not None, f"Warehouse/District not found for W_ID={w_id}, D_ID={d_id}"
            warehouse = row[:6]
            district = row[6:]
        ...
        # Individual DMLs (cannot be easily batched across different tables)
        ...
        if customer[11] == constants.BAD_CREDIT:
            ...
            # For good credit, C_DATA is not updated (or cleared by the benchmark)
            # The original v2 driver sets c_data = "" then passes it to updateBCCustomer which doesn't use it for GCCustomer.
            # The reference sets c_data = "" but updateGCCustomer doesn't use it.
    ...
    def doStockLevel(self, params):
        ...
        # Fetches D_NEXT_O_ID and calculates the stock count in a single query
        self.cursor.execute(q["getStockCount"], [threshold, w_id, d_id, w_id, d_id])
        result = self.cursor.fetchone()
        assert result is not None, f"Stock count query returned no result for W_ID={w_id}, D_ID={d_id}"
