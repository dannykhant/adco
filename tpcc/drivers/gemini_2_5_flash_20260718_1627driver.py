...
TXN_QUERIES = {
    "DELIVERY": {
        # COMBINING_QUERIES: Fetches all necessary info for delivery in one go.
        # This includes finding the lowest new order for each district,
        # joining with ORDERS to get O_C_ID, and calculating OL_AMOUNT sum.
        "batchNewOrders": """
            ...
            FROM (SELECT NO_D_ID, MIN(NO_O_ID) AS NO_O_ID
                  FROM NEW_ORDER
                  WHERE NO_W_ID = %s AND NO_O_ID > -1 -- TPC-C specific to ensure active new orders
                  GROUP BY NO_D_ID) n
            JOIN ORDERS o ON o.O_ID = n.NO_O_ID
                         AND o.O_D_ID = n.NO_D_ID
                         AND o.O_W_ID = %s
            ORDER BY n.NO_D_ID -- Consistent order for results
        """,
    },
    "NEW_ORDER": {
        # CONCURRENCY: Batch read all item info for selected items.
        "batchItemInfo": "SELECT I_ID, I_PRICE, I_NAME, I_DATA FROM ITEM WHERE I_ID IN (%s)",
        # COMBINING_QUERIES & JOIN_ORDER_HINTS: Fetches warehouse, district, and customer info in one query.
        # Uses STRAIGHT_JOIN to enforce order: WAREHOUSE -> DISTRICT -> CUSTOMER (smallest first)
        "getMiscInfo": """
        ...
        # CONCURRENCY: Batch read all stock info for selected items and warehouses.
        ...
        # UPDATE STOCK, INSERT ORDER_LINE are handled by batch helpers using multi-row operations.
    },
    "ORDER_STATUS": {
        # COMBINING_QUERIES: Gets customer, their last order, and all order lines in one query when C_ID is known.
        # Using explicit ON clause for the LEFT JOIN with subquery for better clarity and portability.
        ...
        """,
        # For C_LAST lookup, customer selection is application-level, so this remains separate.
        ...
        # After C_ID is determined from C_LAST, this gets the last order and its lines.
    ...
    "PAYMENT": {
        ...
        # This assumes payment is made at the customer's home warehouse/district for these specific fetches.
        ...
        # After C_ID is determined from C_LAST, this gets warehouse and district info.
    ...
    "STOCK_LEVEL": {
        # COMBINING_QUERIES & PREDICATE_PUSHDOWN: Fetches D_NEXT_O_ID via a derived table
        # and applies the OL_O_ID filter before joining with STOCK.
        "getStockCount": """
            ...
            WHERE ol.OL_W_ID = %s AND ol.OL_D_ID = %s
              AND ol.OL_O_ID < b.high
              AND ol.OL_O_ID >= b.low
              AND s.S_QUANTITY < %s
        """,
    },
}
...
class Gemini_2_5_Flash_20260718_1627Driver(AbstractDriver):
    DEFAULT_CONFIG = {
    ...
    def __init__(self, ddl):
        ...
        if os.path.exists(mysql_ddl):
            ddl = mysql_ddl
        super(Gemini_2_5_Flash_20260718_1627Driver, self).__init__("candidates", ddl)
        self.host = None
        self.port = None
        self.user = None
        self.password = None
    ...
    def makeDefaultConfig(self):
        return Gemini_2_5_Flash_20260718_1627Driver.DEFAULT_CONFIG

    def loadConfig(self, config):
        for key in Gemini_2_5_Flash_20260718_1627Driver.DEFAULT_CONFIG:
...
admin_conn.close()
    ...
    # Batch helpers START (copied verbatim from v2)
    ...
    def _batch_update_customers(self, w_id, updates):
        ...
        for c_id, d_id, ol_total in updates:
            case_whens.append("WHEN C_ID = %s AND C_D_ID = %s THEN C_BALANCE + %s") # TPC-C logic is C_BALANCE + ol_total
    ...
    def doDelivery(self, params):
        ...
        if rows:
            ...
            for row in rows:
                ...
                assert ol_total >= 0.0 # Can be 0 if COALESCE(SUM(), 0)
    ...
    def doNewOrder(self, params):
        ...
        for i_id in i_ids:
            if i_id not in items_map:
                # TPC-C spec says if a non-existent item is rolled back
                self.conn.rollback()
                return None # Indicate transaction failure due to missing item

        # Parameters for getMiscInfo: [d_id, c_d_id (which is d_id), c_id, w_id]
        # v2's parameter ordering is [d_id, d_id, c_id, w_id] and the query reflects this.
        ...
        for i in range(len(i_ids)):
            ...
            if stock_key not in stock_map:
                # TPC-C spec (2.4.2.2 Item Number Check) requires rollback if item not found,
                # but batchItemInfo already handles for non-existent item_id.
                # This branch implies an issue with stock data not item data.
                logging.warn("No STOCK record for (ol_i_id=%d, ol_supply_w_id=%d)" % (ol_i_id, ol_supply_w_id))
                self.conn.rollback() # Rollback on missing stock data
                return None
            ...
            # OL_DELIVERY_D is NULL for New-Order
        ...
        # Return values match TPC-C spec
        customer_info = (c_id, c_last, customer_credit, c_discount)
    ...
    def doOrderStatus(self, params):
        ...
        if c_id is not None:
            # One query to get customer, last order, and order lines
            ...
            # Find customer by last name, then get their last order and lines
            ...
            c_id = customer[0] # Get C_ID of selected customer
    ...
    def doPayment(self, params):
        ...
        if c_id is not None:
            # COMBINING_QUERIES: Get customer, warehouse, and district details in one go.
            # Assumes c_w_id == w_id and c_d_id == d_id for the lookup, per TPC-C spec for specific customer.
            self.cursor.execute(q["getCustomerByCustomerId"], [w_id, d_id, c_id, w_id, w_id, d_id])
            ...
            # Find customer by last name, then pick the median.
            self.cursor.execute(q["getCustomersByLastName"], [w_id, d_id, c_last])
            all_customers = self.cursor.fetchall()
            if not all_customers:
                self.conn.rollback()
            ...
            index = (namecnt - 1) // 2
            customer = all_customers[index]
            c_id = customer[0] # Update c_id for further queries
            c_w_id = w_id # For the c_last path, assume home = payment warehouse
            c_d_id = d_id # For the c_last path, assume home = payment district

            # Get warehouse and district details.
        ...
        if customer[11] == constants.BAD_CREDIT:
            ...
            # c_data remains unchanged for good credit customers
    ...
    def doStockLevel(self, params):
        ...
        # COMBINING_QUERIES and PREDICATE_PUSHDOWN applied in the query directly.
        # D_NEXT_O_ID is fetched via a derived table and used in the WHERE clause for ORDER_LINE.
