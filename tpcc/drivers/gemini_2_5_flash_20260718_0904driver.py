...
TXN_QUERIES = {
    "DELIVERY": {
        "batchNewOrderDetails": """
            SELECT n.NO_D_ID, n.NO_O_ID, o.O_C_ID, SUM(ol.OL_AMOUNT) AS ol_total
            ...
            JOIN ORDERS o ON o.O_ID = n.NO_O_ID
                         ...
                         AND o.O_W_ID = %s
            JOIN ORDER_LINE ol ON ol.OL_O_ID = n.NO_O_ID
    ...
    "NEW_ORDER": {
        ...
        # "updateStock": handled by _batch_update_stock helper
        # "createOrderLine": handled by _batch_insert_order_lines helper
    },
    "ORDER_STATUS": {
        ...
        "getLastOrderWithLines": """
            SELECT lo.O_ID, lo.O_CARRIER_ID, lo.O_ENTRY_D,
                   ol.OL_SUPPLY_W_ID, ol.OL_I_ID, ol.OL_QUANTITY, ol.OL_AMOUNT, ol.OL_DELIVERY_D
            FROM (
                SELECT O_ID, O_CARRIER_ID, O_ENTRY_D, O_W_ID, O_D_ID, O_C_ID
                FROM ORDERS
    ...
    "STOCK_LEVEL": {
        "getStockCount": """
            ...
            FROM (
                SELECT D_NEXT_O_ID - 20 AS low, D_NEXT_O_ID AS high
            ...
            JOIN (
                ...
                WHERE OL_W_ID = %s AND OL_D_ID = %s
                  AND OL_O_ID >= b.low AND OL_O_ID < b.high
            ) AS OL_filtered
            JOIN STOCK AS S ON S.S_I_ID = OL_filtered.OL_I_ID AND S.S_W_ID = %s
...
class Gemini_2_5_Flash_20260718_0904Driver(AbstractDriver):
    DEFAULT_CONFIG = {
    ...
    def __init__(self, ddl):
        ...
        if os.path.exists(mysql_ddl):
            ddl = mysql_ddl
        super(Gemini_2_5_Flash_20260718_0904Driver, self).__init__("candidates", ddl)
        self.host = None
        self.port = None
        self.user = None
        self.password = None
    ...
    def makeDefaultConfig(self):
        return Gemini_2_5_Flash_20260718_0904Driver.DEFAULT_CONFIG

    def loadConfig(self, config):
        for key in Gemini_2_5_Flash_20260718_0904Driver.DEFAULT_CONFIG:
    ...
    def doDelivery(self, params):
        ...
        self.cursor.execute(q["batchNewOrderDetails"], [w_id, w_id])
    ...
    def doNewOrder(self, params):
        ...
        customer_info = misc_row[3:6] # C_DISCOUNT, C_LAST, C_CREDIT
        ...
        for i in range(len(i_ids)):
            ...
            if stock_key not in stock_map:
                ...
                continue # According to TPC-C 2.4.1.5, this should result in rolling back the transaction.
                         # But for simplicity and following previous driver examples, we continue here.
    ...
    def doPayment(self, params):
        ...
        if c_id is not None:
            self.cursor.execute(q["getCustomerDetails"], [d_id, c_id, w_id])
            ...
            index = (namecnt - 1) // 2
            customer = all_customers[index]
            c_id = customer[0] # Update c_id to the one selected
            self.cursor.execute(q["getWarehouseDistrictDetails"], [d_id, w_id])
            row = self.cursor.fetchone()
            warehouse = row[:6]
            district = row[6:]
    ...
    def doStockLevel(self, params):
        ...
        # The parameters for getStockCount are:
        # [D_W_ID (for b), D_ID (for b), OL_W_ID, OL_D_ID, S_W_ID, S_QUANTITY threshold]
        self.cursor.execute(q["getStockCount"], [w_id, d_id, w_id, d_id, w_id, threshold])
